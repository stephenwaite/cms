      ...
      * @author s waite
      * @author Claude
      * aggregate loader: parse 835 like carer304 but record is
      * 242 bytes: adds DT-SENDER (isa06) and DT-PAYERID (ref*2u)
      * for downstream payer plucks. n1*pr name seeds dt-insname,
      * nm1*tt overrides per claim. no carefile I-O. takebacks
      * (negative pay) are recorded with sign. idempotent per
      * remit.
      * S30 835 filein, S35 parmfile, S41 iedidetl (242)
      ...
      * carefile record image, working storage only
       ...
       01  HOLDDETL01.
           ...
           02 HD-SENDER  PIC X(15).
           02 HD-PAYERID PIC X(10).
       01  AMT01.
           02 AMT-0 PIC XXX.
           02 AMT-1 PIC XX.
           02 AMT-2 PIC X(8).
       ...
       01  WS-SENDER  PIC X(15) VALUE SPACES.
       01  WS-PAYERID PIC X(10) VALUE SPACES.
       01  WS-PRNAME  PIC X(30) VALUE SPACES.
       ...
       0005-START.
           ...
           IF WS-DFS = "35"
               ...
               OPEN I-O CAREDETL.
           IF WS-DFS NOT = "00"
               DISPLAY "carer305: caredetl open fs " WS-DFS
                   UPON SYSERR
               STOP RUN.
       P00.
           MOVE SPACE TO FILEIN01 IN-LEG IN-NPI
           READ FILEIN AT END GO TO P99.
      * isa sender id, refreshed per interchange
           IF F1 = "ISA"
               MOVE SPACE TO ISA01
               UNSTRING FILEIN01 DELIMITED BY "*" INTO
                    ISA-0 ISA-1 ISA-2 ISA-3 ISA-4 ISA-5 ISA-6
               MOVE ISA-6 TO WS-SENDER
               GO TO P00.
           IF F1 NOT = "BPR" GO TO P00.
           MOVE SPACE TO WS-PAYERID WS-PRNAME
           MOVE SPACE TO BPR01
           UNSTRING FILEIN01 DELIMITED BY "*" INTO
           ...
           READ FILEIN AT END GO TO P99.
           IF F1 NOT = "TRN"
               DISPLAY "carer305: no TRN after BPR" UPON SYSERR
       ...
       P000.
           MOVE SPACE TO FILEIN01
           READ FILEIN AT END GO TO P99.
      * payer loop: name from n1*pr, payer id from ref*2u,
      * both land here between TRN and N1*PE
      ...
      * payer name as default insname, nm1*tt overrides below
           MOVE WS-PRNAME TO CR-INSNAME
      ...
      * append payment detail to caredetl
       D-DETL.
           ...
           MOVE WS-SENDER  TO DT-SENDER
           MOVE WS-PAYERID TO DT-PAYERID
           IF REVCLM = 1
       ...
       D-DETL-DUP.
           ...
           READ CAREDETL
           IF WS-DFS NOT = "00"
               DISPLAY "carer305: caredetl dup read fs " WS-DFS
                   " " HD-KEY UPON SYSERR
               GO TO D-DETL-EXIT.
      ...
      * same remit re-run, already posted
       ...
       D-DETL-BUMP.
           ...
           ADD 1 TO DT-SEQ
           IF DT-SEQ > 8
               DISPLAY "carer305: caredetl seq overflow " DT-KEY
                   UPON SYSERR
               GO TO D-DETL-EXIT.
           GO TO D-DETL-W.
       D-DETL-EXIT.
       ...
       P99.
           ...
           DISPLAY "carer305: wrote " WS-WROTE
               " dupes " WS-DUPES
