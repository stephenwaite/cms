     ...
     *     BLOCK CONTAINS 6 RECORDS
       ...
       P0.
           ...
           END-IF 
           MOVE SPACE TO ISAM-GARNO
           START FILEISAM KEY NOT < ISAM-KEY INVALID GO TO P0.
           READ FILEISAM NEXT AT END GO TO P0.
           IF (ISAM-PRIPOL NOT = G-PRIPOL)
              AND (ISAM-PRIPOL NOT = G-SECPOL)
              GO TO P0
           END-IF 
      *     DISPLAY "MATCH ON POLICY"
      *     ACCEPT OMITTED           
           IF ISAM-GARNO(1:3) NOT = G-GARNO(1:3) GO TO P0.
      *     DISPLAY "MATCH ON 1ST 3 LETTERS " G-GARNO(1:3)
      *     ACCEPT OMITTED           
           
           IF NOT (G-DOB = ISAM-DOB AND G-SEX = ISAM-SEX) GO TO P0.
      *    DISPLAY "MATCH ON GENDER AND DOB"
      *     ACCEPT OMITTED           
       ...
       P02.    
           ...
           IF (CC-DATE-T < DATE-LOW OR > DATE-HIGH) GO TO P02.
           IF NOT (CC-PLACE = "1" OR "3" OR "5" OR "E" OR "0") 
               GO TO P02
           END-IF    
           ...
           INVALID GO TO P0.
       P1.
           READ PAYCUR NEXT AT END GO TO P02.
           IF PC-KEY8 NOT = CC-KEY8 GO TO P02.
