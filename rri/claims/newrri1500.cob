      ...
      * @license https://github.com/openemr/openemr/blob/master/LICENSE GNU General Public License 3
       ...
       PROGRAM-ID. newrri1500.
       AUTHOR. SID WAITE.
       ENVIRONMENT DIVISION.
       INPUT-OUTPUT SECTION.
       FILE-CONTROL.
           SELECT HOSPFILE ASSIGN TO "S20" ORGANIZATION INDEXED
           ...
           LINE SEQUENTIAL.
           SELECT DOCNAME ASSIGN TO "S85" ORGANIZATION
           LINE SEQUENTIAL.
           ...
           SELECT AUTHFILE ASSIGN TO "S120" ORGANIZATION IS INDEXED
     ...
     *     BLOCK CONTAINS 6 RECORDS
       ...
       01  PROCFILE01.
           02 PROC-KEY.
             03 PROC0-KEY PIC XXXX.
             03 PROC1-KEY PIC X(7).
           02 PROC-TYPE PIC X.
       ...
       01  SV-PAYCODE PIC 999.
       01  CLMTAB01.
           02 CLMTAB PIC X(6) OCCURS 6 TIMES.
       01  LABTAB01.
           02 LABTAB PIC X(4) OCCURS 50 TIMES.
       01  DOCTAB01.
           02 DOCTAB PIC X(20) OCCURS 99 TIMES.
       01  DOCNUM01.
           02 DOCNUM PIC X(8) OCCURS 99 TIMES.
       01  PLACE-TAB01.
      ...
      *     02 FILLER PIC X VALUE SPACE.
       ...
       01  CONTLINE.
           02 CONT-1 PIC X(49).
           02 COMT-2 PIC X(9) VALUE "CONTINUED".
       01 TEST-DATE-TOO.
           02 S-CC PIC XX.
           02 S-YY PIC XX.
           02 S-MM PIC XX.
       ...
       01  DF-FEDID PIC X(14) VALUE SPACE.
       01  DF-GROUP PIC X(12).
       01  DF-GROUPNAME PIC X(29).
       01  CHNL-X PIC X VALUE H"1B".
       PROCEDURE DIVISION.
       Q0. MOVE SPACES TO DOCTAB01 
           MOVE SPACE TO LINE-00 LINE-0 LINE-1 LINE-2 LINE-20
           LINE-2 LINE-3 LINE-4 LINE-41 LINE-42 LINE-43 LINE-44
           LINE-45 LINE-61 LINE-7LEG LINE-7 LINE-8 LINE-9 LINE-11
           LINE-12 LINE-13 LINE-14 LINE-16 L13-PLNAME L14-PLSTREET
           L15-PLCITY L15-PLSTATE L15-PLZIP.
           PERFORM BLANK-1 VARYING X FROM 1 BY 1 UNTIL X > 6.
           OPEN INPUT DOCFILE DOCNAME HOSPFILE 
                      PLACEFILE AUTHFILE ADDRFILE.
           
       Q1. READ DOCNAME AT END GO TO P00.
           MOVE SPACE TO RIGHT-8
           MOVE DN2 TO RIGHT-8
           INSPECT RIGHT-8 REPLACING LEADING " " BY "0"
           MOVE RIGHT-8 TO DOCNUM(DN1)
           MOVE DN3 TO DOCTAB(DN1) GO TO Q1.
       ...
       P000.
           OPEN OUTPUT FILE-OUT
                INPUT INSFILE MPLRFILE GAPFILE PARMFILE PATFILE
                      PAYCUR DIAGFILE PROVCAID COMPFILE.
           MOVE CHNL-X TO CHNL-4
           READ PARMFILE.
           ...
           MOVE L16F2 TO DF-GROUP
           READ PARMFILE. MOVE PF1 TO L12F1.
           MOVE PF1 TO DF-GROUPNAME.
           ...
           MOVE DATE-X TO TEST-DATE-TOO.
           MOVE S-MM TO L15F1
           MOVE S-DD TO L15F2
           MOVE S-CC TO L15F3CC
           MOVE S-YY TO L15F3.
           MOVE 001 TO CC-PAYCODE.
       P1. READ FILEIN AT END PERFORM A7 PERFORM A5 PERFORM A6 GO TO
           P10.
           MOVE FI-KEY TO CHARCUR-KEY.
           READ CHARCUR INVALID DISPLAY FI-KEY " NOT THERE" GO TO P1.
       P1-1.
           MOVE SPACE TO L00F1 L00F2 L00F3 L00F4 L00F5
           IF NOT (FI-PC = 004 OR 028 OR 035 OR 068 OR 264
                               OR 003 OR 141)
           MOVE "X" TO L00F5.
           IF FI-PC = 003 MOVE "X" TO L00F1.
           IF FI-PC = 004 OR 028 OR 035 OR 068 OR 264
               MOVE "X" TO L00F2.
           ...
           READ MPLRFILE INVALID MOVE SPACE TO
           MPLRFILE01.
           MOVE SPACE TO ADDRFILE01
           IF (CC-PAYCODE = 91) AND (G-ADDRCODE NOT = SPACE)
             MOVE G-ADDRCODE TO ADDR-KEY
             READ ADDRFILE INVALID MOVE SPACE TO ADDRFILE01
             END-READ
           END-IF

           MOVE 0 TO FLAG SNUM6 CLAIM-TOT PRIM-PAID.
           IF CC-PATID8 = "P" PERFORM A1 THRU A1-EXIT
           ELSE PERFORM A2 THRU A2-EXIT.
           IF FLAG = 1 GO TO P1.
           ...
           MOVE "X" TO L2F2.
           IF (PAT-RELATE = "2" OR "K") AND (PAT-RELATE NOT = INSRELATE)
           MOVE "X" TO L2F3.
           IF (PAT-RELATE = "4" OR "M") MOVE "X" TO L2F4.
           IF (PAT-RELATE = "5" OR "N") MOVE "X" TO L2F5.
           MOVE CC-DOCP TO HOLD-DOC
           MOVE CC-DATE-T TO HOLD-DATE-T
           MOVE CC-PATID TO HOLD-PATID
           MOVE CC-NEIC-ASSIGN TO HOLD-ASSIGN
           MOVE CC-PAYCODE TO INS-KEY HOLD-INS
           ...
           IF CC-PAYCODE = 91 OR 95
           PERFORM COMPLINE THRU COMPLINE-EXIT.
           IF COMPFILE01 NOT = SPACE GO TO P1-2.
           IF CC-PAYCODE = 91 PERFORM WORKLINE THRU WORKLINE-EXIT.
           IF CC-PAYCODE = 62 PERFORM GAPLINE0 THRU GAPLINE0-EXIT.

           MOVE SPACE TO L42F8 L42F9
           IF (CC-PAYCODE = 91 OR 95) 
           AND (MPLRFILE01 = SPACE)
           AND (ADDRFILE01 NOT = SPACE)
           PERFORM ADDRLINE THRU ADDRLINE-EXIT.
           IF (CC-PAYCODE = 76 OR 91)
            AND (CC-PLACE = "C" OR "M" OR "R")
            AND (MPLRFILE01 = SPACE)
           PERFORM CHCRRLINE THRU CHCRRLINE-EXIT.
       P1-2.
           IF G-PRINS = FI-PC PERFORM PRI-FILL PERFORM SEC-INS
           GO TO HERE-1.
           IF G-SEINS = FI-PC PERFORM SEC-FILL PERFORM PRI-INS
           GO TO HERE-1.
           IF MPLR-TRINS = FI-PC PERFORM TRI-FILL
           PERFORM SEC-INS GO TO HERE-1.
       ...
       HERE-1.
           ...
           IF CC-PAYCODE = "091" MOVE "X" TO L42F3 L43F7 L44F3
           MOVE SPACE TO L42F3.
           IF CC-DAT1 = ZERO MOVE SPACE TO LINE-61
           ELSE MOVE CC-DAT1 TO TEST-DATE-TOO
           MOVE S-MM TO L61F1
           MOVE S-DD TO L61F2
           ...
           L71F2 L71F3 L71F4CC L71F4
           ELSE MOVE CC-DATE-M TO TEST-DATE-TOO
           MOVE S-MM TO L7F3
           MOVE S-DD TO L7F4
           MOVE S-CC TO L7F5CC
           MOVE S-YY TO L7F5
           ...
           MOVE REF-UPIN TO L7F2LEG.
           MOVE REF-NPI TO L7F2.
           MOVE REF-CDNUM TO PROV-KEY
           READ PROVCAID INVALID
           MOVE SPACE TO L7F2-QUAL
           MOVE SPACE TO L7F2LEG
           NOT INVALID
      *     MOVE "ZZ" TO L7F2-QUAL
      *     MOVE PROV-TAX TO L7F2LEG
           MOVE SPACE TO L7F2-QUAL
           MOVE SPACE TO L7F2LEG
           END-READ.
           MOVE CC-PAYCODE TO INS-KEY
           READ INSFILE INVALID MOVE SPACE TO INS-NEIC.
           MOVE SPACE TO L82F3
           IF CC-AUTH = "1" PERFORM AUTH-READ.
           PERFORM A3.
           MOVE 0 TO CNTR LCNTR LAB-TOT LAB-TOTZ DGX.
           MOVE SPACE TO L71 L72
           MOVE CC-PAYCODE TO SV-PAYCODE
           GO TO P3.
       P2.
           READ FILEIN AT END PERFORM A7 PERFORM A5 PERFORM A6
           GO TO P10.
           MOVE FI-KEY TO CHARCUR-KEY.
           READ CHARCUR INVALID DISPLAY "NO KEY" GO TO P2.
      *    DISPLAY FI-ASSIGN " " CC-NEIC-ASSIGN
      *    DISPLAY FI-PATID " "  CC-PATID
           IF ( CC-NEIC-ASSIGN NOT = HOLD-ASSIGN) 
           OR (CC-DOCP NOT = HOLD-DOC)
           OR (CC-DATE-T NOT = HOLD-DATE-T)
           OR (CC-PAYCODE NOT = HOLD-INS) 
           OR (CC-PATID NOT = HOLD-PATID) 
           PERFORM A7 PERFORM A5 PERFORM A6
           MOVE SPACE TO LINE-41 LINE-42 LINE-43 LINE-44 LINE-45
           LINE-81 LINE-82 L13-PLNAME L14-PLSTREET L15-PLCITY 
           L15-PLSTATE L15-PLZIP LINE-61 LINE-7LEG LINE-7 LINE-8
              GO TO P1-1.
       ...
       P3.
           ...
           MOVE SPACE TO L10-LADY(CNTR)
           IF INS-CAID = "\/" MOVE "5" TO L10-LADY(CNTR).
           ...
           MOVE CC-DATE-T TO TEST-DATE-TOO
           MOVE S-MM TO L10F1(CNTR)
           MOVE S-DD TO L10F2(CNTR)
           MOVE S-YY TO L10F3(CNTR).
           MOVE S-MM TO L10F1S(CNTR)
           MOVE S-DD TO L10F2S(CNTR)
           MOVE S-YY TO L10F3S(CNTR)
           IF CC-PROC1 = "Y9872" OR "Y9873" OR "J9999" OR "99070"
           ...
           PERFORM DF-SEARCH.
           IF CC-PL = "2" MOVE SPACE TO L13-PLNAME
           MOVE G-STREET TO L14-PLSTREET 
           MOVE G-CITY TO L15-PLCITY 
           MOVE G-STATE TO L15-PLSTATE 
           MOVE G-ZIP TO L15-PLZIP.
           ...
           IF CC-PL = "E" MOVE "23" TO L10F4(CNTR) GO TO LS1.
           IF CC-PL = "K" MOVE "31" TO L10F4(CNTR).
           IF CC-PL = "Q" MOVE "13" TO L10F4(CNTR).
           IF CC-PL = "D" MOVE "33" TO L10F4(CNTR).
           IF SUBMITTER-ID  = "CC" GO TO LS1.
           IF CC-PROC1 > "99200" AND < "99206" MOVE "11" TO L10F4(CNTR).
           IF CC-PROC1 > "99210" AND < "99216" MOVE "11" TO L10F4(CNTR).
           IF CC-PROC1 > "99216" AND < "99221" MOVE "22" TO L10F4(CNTR).
           ...
           IF CC-PROC1 > "99240" AND < "99246" MOVE "11" TO L10F4(CNTR).
      ...
      *     IF CC-PROC1 > "99310" AND < "99316" MOVE "31" TO L10F4(CNTR).
       ...
       LS1.
           ...
           MOVE SPACE TO F101(CNTR) F102(CNTR) F103(CNTR) L10F5(X)
           F105(CNTR) F106(CNTR) F107(CNTR) F108(CNTR) F109(CNTR)
           F1010(CNTR) F1011(CNTR) F1012(CNTR)  F1014(CNTR)
      ...
      *     MOVE DOC-GROUP TO L16F2
           MOVE DOC-FEDID TO L11F1
           MOVE DOC-NAME TO L14F0.
      *     IF DOC-IND = "1" MOVE DOC-PVNUM TO L10F13-LEGNUM(CNTR).
      *     IF DOC-IND = "2" MOVE DOC-UPIN TO L10F13-LEGNUM(CNTR).
      *     IF DOC-IND = "0" MOVE SPACE TO L10F13-LEGNUM(CNTR).
       DOC-1-EXIT.
           EXIT.
       ...
       AUTH-READ.
           ...
           READ AUTHFILE INVALID MOVE SPACE TO AUTH-NUM.
           IF L82F3 = SPACE MOVE AUTH-NUM TO L82F3.
       A3.
      ...
      *     WRITE OUT01 FROM LINE-7LEG AFTER 1.
       ...
       A9.
           ...
           WRITE OUT01 FROM LINE-82 AFTER 1.
           MOVE SPACE TO OUT01
           WRITE OUT01 AFTER 3.
       A7.
       ...
       A8.
           IF LCNTR > 5 MOVE SPACE TO OUT01 WRITE OUT01
             IF FI-ASSIGN = "A" MOVE " X " TO L11F5 
               MOVE SPACE TO L11F6 
               ELSE MOVE SPACE TO L11F5 MOVE " X " TO L11F6
             END-IF
           MOVE LINE-11 TO CONT-1
           WRITE OUT01 FROM CONTLINE
           MOVE 0 TO LCNTR PERFORM A6 PERFORM A3 PERFORM A9.
      *     MOVE SPACE TO LINE-1002LEG(X)
           MOVE SPACE TO OUT01
           WRITE OUT01 AFTER 1.
      ...
      *     PERFORM S4 THRU S4-EXIT.
       ...
       S7. READ PAYCUR NEXT AT END GO TO S4-EXIT.
           ...
           END-IF.
           MOVE PC-DATE-T TO TEST-DATE-TOO.
           MOVE S-MM TO L101F1
           MOVE S-DD TO L101F2
           ...
           MOVE INS-NAME TO L101F4.
           IF (PC-DENIAL = "14")
           OR (PC-PAYCODE = "007" OR "008" OR "009" OR "014")
           MOVE "INS REDUC" TO L101F4 ELSE MOVE "PAYMT" TO L101F41.
           MOVE PC-AMOUNT TO L101F5.
           IF LCNTR > 5
             IF FI-ASSIGN = "A" MOVE " X " TO L11F5 
               MOVE SPACE TO L11F6 
               ELSE MOVE SPACE TO L11F5 MOVE " X " TO L11F6
             END-IF
           MOVE LINE-11 TO CONT-1
           WRITE OUT01 FROM CONTLINE AFTER 2
           MOVE 0 TO LCNTR PERFORM A6
           PERFORM A3 PERFORM A9.
           WRITE OUT01 FROM LINE101 AFTER 2.
           ADD 1 TO LCNTR. 
           IF SV-PAYCODE = "005" AND PC-PAYCODE = G-PRINS
           GO TO S4-EXIT.
           GO TO S7.
       S4-EXIT. EXIT.
       A5.
           ...
           ELSE WRITE CHNL-4 FROM CHNL-X AFTER 1.
      ...
      *    WRITE OUT01 FROM LINE-111
       ...
       BLANK-1. MOVE SPACE TO F101(X) F102(X) F103(X) L10F5(X)
           ...
           F1011(X) F1012(X)  F1014(X) L10F1LEG(X).
       WORKLINE.
           MOVE CC-PAYCODE TO L0F1
       ...
       PRI-INS.
           ...
           MOVE SPACE TO L45F1 L45F2 L43F4 L43F5.
       ...
       GAPLINE.
           ...
           MOVE G-PR-GROUP TO GAPKEY
           READ GAPFILE INVALID MOVE SPACE TO GAP-NAME DISPLAY "BAD".
           IF CC-PAYCODE = 062
           MOVE GAP-NAME TO L0F2
           MOVE GAP-ADDR TO L0F3
           MOVE GAP-CITY TO L0F4
           ...
           MOVE GAPKEY TO ALF-72
           MOVE "##" TO ALF-71 MOVE ALF-7 TO L45F1-A.
       GAPLINE-EXIT. EXIT.
       ADDRLINE.
           MOVE CC-PAYCODE TO L0F1
           MOVE ADDR-NAME TO L0F2
           MOVE ADDR-STREET TO L0F3
           MOVE ADDR-CITY TO L0F4
           MOVE ADDR-STATE TO L0F5
           MOVE ADDR-ZIP TO L0F6.
       ADDRLINE-EXIT. EXIT.    

       CHCRRLINE.
           MOVE CC-PAYCODE TO L0F1
           MOVE SPACE TO HOSP-KEY
           IF L0F1 = G-PRINS
             MOVE G-PR-GROUP TO HOSP-KEY
           END-IF
           IF L0F1 = G-SEINS
           MOVE G-SE-GROUP TO HOSP-KEY
           END-IF
           READ HOSPFILE INVALID 
           MOVE SPACE TO HOSPFILE01.
           MOVE HOSP-NAME TO L0F2
           MOVE HOSP-BOX TO L0F3
           IF L0F3 = SPACE
           MOVE HOSP-STREET TO L0F3.
           MOVE HOSP-CITY TO L0F4
           MOVE HOSP-STATE TO L0F5
           MOVE HOSP-ZIP TO L0F6.
       CHCRRLINE-EXIT. EXIT.    
       ...
       COMPLINE.
           ...
           READ COMPFILE INVALID 
           MOVE SPACE TO COMPFILE01 GO TO COMPLINE-EXIT.
           MOVE COMP-INSNAME TO L0F2
           MOVE COMP-INSADDR2 TO L0F3
           IF L0F3 = SPACE
           MOVE COMP-INSADDR1 TO L0F3.
           MOVE COMP-INSCITY TO L0F4
           MOVE COMP-INSSTATE TO L0F5
           MOVE COMP-INSZIP TO L0F6.
       COMPLINE-EXIT. EXIT.    
       
       P10. CLOSE FILE-OUT CHARCUR GARFILE PATFILE REFPHY PROCFILE
            GAPFILE HOSPFILE ADDRFILE.
           DISPLAY "# OF CLAIMS = " NUM-CLAIM. STOP RUN.
