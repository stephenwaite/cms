     ...
     *     BLOCK CONTAINS 6 RECORDS
      ...
      * HOLDS THE POSITION OF A FIELD WITHIN IT'S DATA SET
       ...
       01 CCLEN-TAB01-RE.
           02 FILLER PIC X(30) VALUE "110806010507060302030102080602".
           02 FILLER PIC X(24) VALUE "240108010106010105050801".
           02 FILLER PIC X(12) VALUE "010802020101".
       01 CCLEN-TAB01 REDEFINES CCLEN-TAB01-RE.
           10 CCLEN-TAB   PIC 99 OCCURS 33 TIMES.
       01  CCDES-CONSTANT.
               10 FILLER PIC X(24) VALUE "REC NO  PATNO   CLAIM   ".
               10 FILLER PIC X(24) VALUE "TYP SERVDIAGNOSEPROCEDUR".
       ...
       01  CCGD-TABLE REDEFINES CCDES-CONSTANT.
             05 CCDES-KEY PIC X(8) OCCURS 33 TIMES INDEXED BY CCINDX.
       01  INPUT-DATE.
           05 T-MM  PIC 99.
           05 T-DD  PIC 99.
           05 T-CC  PIC 99.
      ...
      *     DISPLAY "TO USE BATCH METHOD TYPE BAT".
       1000-ACTION.
           ...
           CALL "/home/sidw/tri001.a" USING GARPAT1
           MOVE 1 TO GARPAT1
           OPEN INPUT GARFILE PATFILE MPLRFILE INSFILE GAPFILE
           GO TO 1000-ACTION.
           IF DATAIN = "AC"
           CLOSE CHARFILE CHARCUR PAYFILE
           CALL "/home/sidw/tri002.a" USING CHAR1
           MOVE 1 TO CHAR1
           OPEN INPUT CHARFILE 
           OPEN I-O CHARCUR PAYFILE
           GO TO 1000-ACTION.
      *     IF DATAIN = "NONBAT" GO TO 999-A.
      *     IF DATAIN = "BAT" CALL "/usr/sidw/tri007.a" 
      ...
      *    IF DATAIN = "RC" GO TO P0.
           ...
           DISPLAY "PB TO PRINT BILL FOR THIS ACCT."
           ...
           CALL "/home/sidw/tri011.a" USING ACTION ALF-8 
           GO TO 1000-ACTION.
           IF ACTION = "HS" OR "HSC" OR "HSP"
           MOVE PD-KEY8 TO ALF-8 CALL "/home/sidw/tri011.a"
           ...
           CLOSE CMNTFILE CALL "/home/sidw/tri000.a" USING ALF-8
           ...
           CALL "/home/sidw/tri205sid4.acu" USING PB1 G-GARNO
           MOVE 1 TO PB1
           OPEN I-O CHARCUR PAYFILE
           GO TO 1000-ACTION.
      ...
      *    SET P-IND DOWN BY 1
       ...
       FC33.
           ...
           DISPLAY Z " " ALF-5 " " DISPLAY-DATE " PC " CC-PAYCODE
      ...
      *     ADD 1 TO Z
       ...
       FC55.
           ...
           DISPLAY Z " " ALF-5 " " DISPLAY-DATE " PC " CD-PAYCODE
      ...
      *    ISQUIET PAYFILE WITH 1 AND 3
       ...
       1400-CHANGE-PROCESS.
           ...
           IF DATAIN = "UP"  GO TO 5000-WRITE-PAYFILE.
           IF DATAIN = "?"
       ...
       2300-DENIAL.
           ...
           DISPLAY "NF = NON-SUFFICIENT FUNDS"
           GO TO 2000TI.
      ...
      *    DISPLAY PAYA-TAB(XX) " " A-TAB(P-IND)
       ...
       1400CPC.
           ...
           UNLOCK CHARCUR RECORD
               DISPLAY "NO CHANGE"  GO TO 1000-ACTION.
           IF DATAIN = "UP"  GO TO 5000-WRITE-CHARCUR.
           ...
           DISPLAY "29=ADMTDT 30=MOD3       31=AUTHNUM 32=ASSIGN" 
           DISPLAY "33=NEICASGM"
           DISPLAY "L TO LIST THE RECORD"
           ...
           OR RIGHT-2 > "33" DISPLAY "INVALID FIELD-CODE"
               GO TO 1400CPC.
       ...
       CC-2150-DIAG.
           ...
           GO TO 4900CPC.
           IF IN-FIELD = "F" GO TO CC-1DIAG-SEARCH.
           MOVE IN-FIELD-5 TO DIAG-KEY.
           READ DIAGFILE INVALID DISPLAY "NOT ON FILE"
           GO TO CC-2000TI.
           DISPLAY DIAG-TITLE.
           MOVE IN-FIELD-5 TO CC-DIAG.
           GO TO 4900CPC.
       CC-1DIAG-SEARCH.
           DISPLAY "SEARCH KEY ?".
           ACCEPT IN-FIELD
           IF IN-FIELD = "?"
           DISPLAY "TYPE BEGINNING DIGITS OR DESCRIPTION"
           DISPLAY "OR A <CR> TO END THE SEARCH"
           GO TO CC-1DIAG-SEARCH.
           IF IN-FIELD = SPACE GO TO CC-2000TI.
           MOVE 0 TO X
           IF ((IN-FIELD-TAB(1) NUMERIC) 
                AND (IN-FIELD-TAB(2) ALPHABETIC)) 
           OR ((IN-FIELD-TAB(1) = "V") 
                AND (IN-FIELD-TAB(2) NOT NUMERIC))
           OR ((IN-FIELD-TAB(1) ALPHABETIC) 
                AND (IN-FIELD-TAB(2) ALPHABETIC)) 
           GO TO CC-2DIAG.
           MOVE SPACE TO DIAG-KEY
           MOVE IN-FIELD-5 TO DIAG-KEY
           START DIAGFILE KEY NOT < DIAG-KEY
           INVALID GO TO CC-1DIAG-SEARCH.
           GO TO CC-3DIAG.
       CC-2DIAG.
           MOVE SPACE TO DIAG-TITLE
           MOVE IN-FIELD-5 TO DIAG-TITLE
           START DIAGFILE KEY NOT < DIAG-TITLE
           INVALID GO TO CC-1DIAG-SEARCH.

       CC-3DIAG. READ DIAGFILE NEXT AT END
       ...
       CC-2-DX2.
           ...
           DISPLAY "OR A <CR> IF NO DIAGNOSIS IS NECESSARY."
               GO TO CC-2000TI.
           IF IN-FIELD = "00000" OR SPACES MOVE ZEROES TO CC-DX2
           GO TO 4900CPC.
           IF IN-FIELD = "F" GO TO CC-1DIAG-SEARCH.
           MOVE IN-FIELD-5 TO DIAG-KEY.
           READ DIAGFILE INVALID DISPLAY "NOT ON FILE"
       ...
       CC-2-DX3.
           ...
           DISPLAY "OR A <CR> IF NO DIAGNOSIS IS NECESSARY."
               GO TO CC-2000TI.
           IF IN-FIELD = "00000" OR SPACES MOVE ZEROES TO CC-DX3
           GO TO 4900CPC.
           IF IN-FIELD = "F" GO TO CC-1DIAG-SEARCH.
           MOVE IN-FIELD-5 TO DIAG-KEY.
           READ DIAGFILE INVALID DISPLAY "NOT ON FILE"
       ...
       CC-2-AUTH.
           IF IN-FIELD = "?"
           DISPLAY "1= ADD  2= CHANGE   A PRIOR AUTHORIZATION NUMBER"
           DISPLAY "<CR> = NO PRIOR AUTHORIZATION" 
           DISPLAY "0= UNSET AN EXISTING AUTHORIZATION NUMBER"
           DISPLAY "4= RESET AN EXISTING AUTHORIZATION NUMBER"
           GO TO CC-2000TI.
           IF IN-FIELD = SPACE GO TO 4900CPC.
           IF IN-FIELD-2 = "0 " MOVE "0" TO CC-AUTH
           GO TO 4900CPC.
           IF IN-FIELD-2 NOT = "1 " AND NOT = "2 "  AND NOT = "4 "
           DISPLAY "INVALID" GO TO CC-2000TI.
           MOVE 0 TO FLAG 
           MOVE CC-AUTH TO ALF-1
           PERFORM AUTH-1 THRU AUTH-1-EXIT
           IF FLAG = 2 DISPLAY "CHANGED" 
           MOVE "1" TO CC-AUTH GO TO 4900CPC.
           IF FLAG = 1 DISPLAY "ADDED" 
           MOVE "1" TO CC-AUTH GO TO 4900CPC.
           MOVE ALF-1 TO CC-AUTH
           GO TO 4900CPC.
       ...
       5000-WRITE-CHARCUR.
           ...
           GO TO CC-2000TI.
      *    IF CC-PROC > "ZZ     " AND < "ZZZZZZZ"
      *    DISPLAY "OLD PROCEDURE CODE MUST BE CHANGED FIRST"
      *    GO TO 1000-ACTION.
      ...
      *    ISQUIET CHARCUR WITH 1 AND 1
       ...
       LC-1.
           ...
           "  " CC-DIAG " " CC-DX2 " " CC-DX3 " " CC-PROC " " CC-MOD2
           "  " CC-MOD3  NEF-8

           DISPLAY "KEY         PATIENT  CLAIM  PL DX1   DX2   DX3   "
           "PROC   MOD2 MOD3   $$$"
           DISPLAY " "
           ...
           DISPLAY "ASGN CLM-ASGN COLLT  PAPER  STATUS   CLM-DATE AUTH"
           "     ADMIT-DT"
      ...
      -    " DATE VTCAID# REFPRMT".
       ...
       10-PR. 
           ...
           MOVE "4" TO FILEOUT01.
           IF CC-PAYCODE = "003" OR "028" OR "062"
           MOVE "3" TO FILEOUT01.
           WRITE FILEOUT01
           MOVE SPACE TO FILEOUT01
           STRING CC-PAYCODE CC-KEY8 CHARCUR-KEY CC-DATE-T CC-ASSIGN
           CC-PLACE CC-DOCP DELIMITED BY "!!!!" INTO FILEOUT01
           WRITE FILEOUT01.
           CLOSE FILEOUT
      *     OPEN INPUT FILEOUT
      ...
      *     DISPLAY "PAY TRANSACTION MAINTENANCE PROGRAM HAS ENDED".
           CLOSE PAYFILE.
           CLOSE CHARCUR.
           CLOSE AUTHFILE.
      *     CLOSE GARFILE PATFILE CHARFILE CMNTFILE INSFILE.
      *     CLOSE DIAGFILE PROCFILE REFPHY.
           STOP RUN.
