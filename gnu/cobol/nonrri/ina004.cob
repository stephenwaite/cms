     ...
     *     BLOCK CONTAINS 6 RECORDS
      ...
      *     IF DATAIN = "BAT" CALL "tri007" 
      *     USING CURRENT-BATCH CBN
      *     GO TO 1000-ACTION.
           IF DATAIN = "CD" PERFORM CD1X GO TO 1000-ACTION.
      ...
      *    IF DATAIN = "RC" GO TO P0.
           ...
           CALL "tri011" USING ACTION ALF-8 
           GO TO 1000-ACTION.
           IF ACTION = "HS" OR "HSC" OR "HSP"
           MOVE PD-KEY8 TO ALF-8 CALL "tri011"
           ...
           CLOSE CMNTFILE CALL "tri000" USING ALF-8
      ...
      *    GO TO WO4.
       ...
       WO14. MOVE PD-AMOUNT TO NEF-4 DISPLAY "REDUCTION " NEF-4.
      ...
      *    GO TO WO4.
       ...
       WH14. MOVE PD-AMOUNT TO NEF-4 DISPLAY "WITHHOLD " NEF-4.
           MOVE PAYBACK01 TO PAYFILE01 MOVE XYZ TO PD-KEY3.
      ...
      *     GO TO 1000-ACTION.
       ...
       LD1. IF DF-PAYCODE = "000" NEXT SENTENCE
           ...
           ELSE MOVE DF-AMOUNT TO NEF-4
           DISPLAY "AMOUNT " NEF-4.
       ...
       FP1.
           ...
           SET P-IND TO 1
           IF CD1 = 1 PERFORM CD-SET THRU CD-EXIT.
           IF IN-FIELD = "FCC" OR ACTION = "FCC"
           DISPLAY "CHRGS ONLY" GO TO FC1.
           START PAYCUR KEY NOT < PAYCUR-KEY INVALID GO TO FP5.
      ...
      *    IF P-IND > 1
      *    SET P-IND DOWN BY 1
           START PAYFILE KEY NOT < PAYFILE-KEY INVALID GO TO FC1.
      ...
      *     MOVE PD-AMOUNT TO A-TAB(P-IND)
       ...
       FC3. READ CHARCUR NEXT AT END GO TO FC6.
           ...
           MOVE CC-AMOUNT TO TOT-AMOUNT NEF-4
           MOVE CC-CLAIM TO CLAIM
       ...
       FC33.
           ...
           " CLAIM " CC-CLAIM " AMOUNT " NEF-4 " HS " CC-PROC 
           " " CC-DIAG " " CC-DOCP
       ...
       FC6. MOVE G-GARNO TO CD-KEY8 MOVE "000" TO CD-KEY3.
           START CHARFILE KEY NOT < CHARFILE-KEY INVALID GO TO FP1-EXIT.
       FC5. READ CHARFILE NEXT AT END GO TO FP1-EXIT.
           ...
           ELSE GO TO FC5.
           IF (CD1 = 0)
           OR ((CD1 = 1) AND (DATE-OF-CHARGE = CD-DATE-T OR SPACE))
           NEXT SENTENCE
           ELSE GO TO FC5.
           IF (IN-FIELD = "FT") OR (ACTION = "FT")
           ...
           MOVE CD-AMOUNT TO TOT-AMOUNT NEF-4
           MOVE CD-CLAIM TO CLAIM
      ...
      *     ADD 1 TO Z
       ...
       FPS. IF C-TAB(XIND) = CLAIM
           ...
           MOVE A-TAB(XIND) TO NEF-4
           MOVE NEF-4 TO ALF-11
           MOVE TOT-AMOUNT TO NEF-4
       ...
       1200-FIND. START PAYFILE KEY NOT < PAYFILE-KEY INVALID
      ...
      *    ISQUIET PAYFILE WITH 1 AND 3
       ...
       4000-DATE-T.
           ...
           MOVE PAYBACK01 TO PAYFILE01
           GO TO 2000TI.
           IF IN-FIELD-8 = SPACES ACCEPT PD-DATE-T FROM DATE YYYYMMDD
           GO TO 4900DEE.
           IF IN-FIELD-2 NUMERIC AND
           IN-FIELD-TAB(3) = " " AND IN-FIELD-TAB(4) = " "
           AND IN-FIELD-TAB(5) = " " AND IN-FIELD-TAB(6) = " "
      ...
      *     IF IN-FIELD-2 = "14" DISPLAY "INS. REDUCTION" BELL0.
       ...
       CUR-1.
           ...
           START CHARCUR KEY NOT < CHARCUR-KEY INVALID GO TO CUR-END.
       ...
       DAL-1.
           ...
           START CHARFILE KEY NOT < CHARFILE-KEY INVALID GO TO DAL-END.
       ...
       DC1. MOVE "000" TO CC-KEY3 MOVE G-GARNO TO CC-KEY8.
           ...
           GO TO DC1-EXIT.
           START CHARCUR KEY NOT < CHARCUR-KEY INVALID GO TO DC5.
       ...
       DC5.   MOVE "000" TO CD-KEY3 MOVE G-GARNO TO CD-KEY8.
           START CHARFILE KEY NOT < CHARFILE-KEY INVALID GO TO DC-SORT.
      ...
      *    DISPLAY TOT-BAL " " TOT-AMOUNT.
       ...
       SD2. IF D-TAB(AA) < D-TAB(YY)
        ...
        SD3-1.
           ...
           PERFORM DR1-0 THRU DR1-EXIT
           SET XX TO P-IND
           ELSE PERFORM DR1-0 THRU DR1-EXIT
      ...
      *    DISPLAY PAYA-TAB(XX) " " A-TAB(P-IND)
       ...
       DR2. MULTIPLY DR1 BY -1 GIVING DR1.
           IF DR1 = 1
       ...
       WO6. MOVE G-GARNO TO CC-KEY8 MOVE "000" TO CC-KEY3.
           MOVE 1 TO FLAG.
           MOVE 0 TO TOT-CLAIM.
           START CHARCUR KEY NOT < CHARCUR-KEY INVALID GO TO WO81.
       ...
       WO81. MOVE G-GARNO TO CD-KEY8 MOVE "000" TO CD-KEY3.
           START CHARFILE KEY NOT < CHARFILE-KEY INVALID GO TO WO12.
       ...
       WO8. MOVE G-GARNO TO PC-KEY8 MOVE "000" TO PC-KEY3.
           START PAYCUR KEY NOT < PAYCUR-KEY INVALID GO TO WO10.
       ...
       WO10. MOVE PD-CLAIM TO IN-FIELD-6.
           ...
           START PAYFILE KEY NOT < PAYFILE-KEY INVALID GO TO WO12.
       ...
       LPAY-1. DISPLAY PAYFILE-KEY " " PD-NAME " "
           MOVE PD-AMOUNT TO NEF-4
           DISPLAY "3 = " NEF-4 " 4 = " PD-PAYCODE
      ...
      *     MOVE "?" TO IN-FIELD GO TO CC-4000-DATE-T.
       ...
       CC-2-DATE-A.
           ...
           GO TO CC-2000TI.
           IF IN-FIELD-8 = "T" ACCEPT CC-DATE-A FROM DATE YYYYMMDD
           GO TO 4900CPC.
      ...
      *    ISQUIET CHARCUR WITH 1 AND 1
           DISPLAY "UPDATE MADE"  GO TO 1000-ACTION.
       CC-1200-FIND. START CHARCUR KEY NOT < CHARCUR-KEY INVALID
           DISPLAY " END OF FILE" GO TO 1000-ACTION.
           MOVE "D" TO UPDOWN.
       CC-1200-FIND-20.
           MOVE 0 TO X.
       ...
       CC-1200-S1.
           MOVE CC-AMOUNT TO NEF-4
           MOVE CC-DATE-T TO TEST-DATE
           MOVE CORR TEST-DATE TO DISP-DATE
           DISPLAY X " " CHARCUR-KEY " " DISP-DATE " " 
           CC-PROC CC-MOD2 " " NEF-4 " " CC-PAYCODE " " CC-CLAIM " " 
           CC-PAPER " " NAME14
      ...
      *         DX5     DX6"
       ...
       CC-1.
           ...
           INVALID DISPLAY "NO RECORDS"  GO TO 1000-ACTION.
       CC-2. READ CHARCUR NEXT WITH LOCK AT END GO TO 1000-ACTION.
           ...
           MOVE CC-DATE-T TO TEST-DATE
           MOVE CORR TEST-DATE TO DISP-DATE
           DISPLAY CC-PAYCODE " " CC-PROC " " DISP-DATE " " NEF-4.
       ...
       RA-1-1.   
           ...
           START CHARCUR KEY NOT < CHARCUR-KEY INVALID 
            DISPLAY "NO RECORDS" GO TO 1000-ACTION.
