      ...
      *
       ...
       INS-LOOK.
           ...
           GO TO INS-CP.
       ADDR-LOOK.
           IF ACTION = "FS" PERFORM ADDR-1 THRU ADDR-1-EXIT
           GO TO 1000-ACTION.
           MOVE SPACE TO RIGHT-4
           UNSTRING GARNO DELIMITED BY " " INTO RIGHT-4
           INSPECT RIGHT-4 REPLACING ALL " " BY "0".
           MOVE RIGHT-4 TO ADDR-KEY
           READ ADDRFILE INVALID DISPLAY "INVALID"
           GO TO 1000-ACTION.
           IF ADDRSTAT NOT = "00" 
           DISPLAY "STATUS = " ADDRSTAT
           DISPLAY "RECORD BEING USED"
           GO TO 1000-ACTION.
           IF ACTION = "LS" PERFORM LS-1 
           UNLOCK ADDRFILE RECORD
           GO TO 1000-ACTION.
           GO TO ADDR-CP.
       ...
       1220-AP.
           ...
           WRITE GARFILE01 INVALID KEY DISPLAY "NO UPDATE"
      ...
      *    OPEN I-O GARFILE.
       ...
       1400-CP.
           ...
           PERFORM LG-1-0 MOVE GARBACK TO GARFILE01 GO TO 1400-CP.
           IF DATAIN = "?"
       ...
       2245-PRIPOL.
           ...
           GO TO 2000TI.
           IF (PRINS = "002" OR "003" OR "004" OR "064") AND 
      ...
      *    MOVE "A" TO SE-ASSIGN.
       ...
       3245-SECPOL.
           ...
           IF (SEINS = "002" OR "003" OR "004"OR "064") AND 
           (IN-FIELD-9 NOT NUMERIC) 
           DISPLAY "POLICY MUST BE A 9-DIGIT NUMBER"
       ...
       2-TRIPOL.
           ...
           IF (MPLR-TRINS = "002" OR "003" OR "004" OR "064") 
       ...
       5001-WG.
           ...
           REWRITE GARFILE01 INVALID KEY DISPLAY "NO UPDATE."
      ...
      *    DISPLAY IN-FIELD-5
       ...
       2000-AS.    
           PERFORM ADDR-ADD-LOOP VARYING ADDR-A-KEY FROM 1 BY 1
             UNTIL ADDR-A-KEY > 6
           IF IN-FIELD = "X" DISPLAY "NO UPDATE" GO TO 2001-AS.
           WRITE ADDRFILE01 INVALID KEY DISPLAY "NO UPDATE"
           DISPLAY "THIS SHOULD NOT HAVE HAPPENED. CONTACT DATA CENTER."
           DISPLAY "THIS PROGRAM HAS BEEN TERMINATED"
           GO TO 9100-CLOSE-MASTER-FILE.
       ...
       ADDR-CP.
           ...
           IF DATAIN = "X"
               DISPLAY "NO CHANGE"  GO TO 1000-ACTION.
           IF DATAIN = "UP"  GO TO 5000-WRITE-ADDRFILE.
           IF DATAIN = "LS" PERFORM LS-1 GO TO ADDR-CP.
           IF DATAIN = "?"
           DISPLAY "ENTER A FIELD CODE <COMMA> AND NEW DATA."
               DISPLAY "UP = UPDATE CHANGES"
           DISPLAY " X = NO CHANGE"
       ...
       ADDR-INPUT.
           ...
           DISPLAY "OR ENTER DATA NEEDED"
           GO TO ADDR-DISPLAY.
           IF IN-FIELD = "BK" AND ADDR-A-KEY = 1 MOVE "X" TO IN-FIELD
           SET ADDR-A-KEY TO 7 GO TO ADDRDEEX.
           IF IN-FIELD = "BK" GO TO ADDR-BACK.
           IF IN-FIELD = "LS" PERFORM LS-1 GO TO ADDR-DISPLAY.
           IF IN-FIELD = "X" SET ADDR-A-KEY TO 7
           GO TO ADDRDEEX.
           GO TO ADDR-GO-TO.
       ADDR-BACK.
       ...
       ADDR-GO-TO.
           MOVE 0 TO FLAG.
           MOVE ADDR-LEN-TAB(ADDR-INDX) TO Q ADD 1 TO Q.
           IF IN-FIELD-TAB(Q) NOT = " " MOVE 1 TO FLAG.
           IF FLAG = 1 DISPLAY "DATA TOO LONG, MUST NOT BE GREATER "
             "THAN " ADDR-LEN-TAB(ADDR-INDX) ".".
           IF FLAG = 1 AND ACT-1 = "C" GO TO ADDR-CP.
           IF FLAG = 1 AND ACT-1 = "A" GO TO ADDR-DISPLAY.
           GO TO 1-ADDR
            2-ADDR 3-ADDR 4-ADDR 5-ADDR 6-ADDR 
       ...
       1-ADDR.
           ...
           DISPLAY "MUST BE NUMERIC" GO TO ADDR-TI.
           MOVE RIGHT-4 TO ADDR-KEY
           READ ADDRFILE INVALID CONTINUE
           NOT INVALID DISPLAY "ALREADY ON FILE"
           PERFORM LS-1
           GO TO ADDR-TI.
           GO TO ADDRDEE.
       2-ADDR.
       ...
       ADDRDEEX. EXIT.
       5000-WRITE-ADDRFILE.
           REWRITE ADDRFILE01 INVALID KEY DISPLAY "NO UPDATE."
           DISPLAY "THIS SHOULD NOT HAPPEN. CONTACT THE DATA CENTER."
           DISPLAY "THIS PROGRAM IS TERMINATED."
           GO TO 9100-CLOSE-MASTER-FILE.
           UNLOCK ADDRFILE RECORD
      ...
      *  BEGIN PATIENT (DEPENDENT) ROUTINE **
       ...
       ADDR-3. DISPLAY "STARTING POINT?".
           ...
           GO TO ADDR-1-END.
       ADDR-4. READ ADDRFILE NEXT AT END DISPLAY "END OF FILE"
           GO TO ADDR-1-EXIT.
           DISPLAY ADDR-KEY " " ADDR-NAME " " ADDR-STREET " " ADDR-CITY
           " " ADDR-STATE " " ADDR-ZIP
           ADD 1 TO X
           IF X > 8 MOVE 0 TO X DISPLAY "BY " ALF-8 ACCEPT ANS
           IF ANS NOT = SPACE GO TO ADDR-1-EXIT.
           GO TO ADDR-4.
       ADDR-1-END. DISPLAY "END OF FILE".
       ADDR-1-EXIT. EXIT.
